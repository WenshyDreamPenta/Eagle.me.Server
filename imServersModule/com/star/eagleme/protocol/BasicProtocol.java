package com.star.eagleme.protocol;
...
public abstract class BasicProtocol
{
    ...
    // 长度均以字节（byte）为单位
    public static final int LENGTH_LEN = 4;       //记录整条数据长度数�?�的长度
    protected static final int VER_LEN = 1;       //协议的版本长度（其中�?3位作为预留位，后5位作为版本号�?
    protected static final int TYPE_LEN = 1;      //协议的数据类型长�?

    private int reserved = 0;                     //预留信息
    private int version = Config.VERSION;         //版本�?
    ...
    /**
     * 拼接发�?�数据，此处拼接了协议版本�?�协议类型和数据长度，具体内容子类中再拼�?
     * 按顺序拼�?
     *
     * @return
     */
    ...
    {
        byte r = data[LENGTH_LEN];//�?4个字节（0�?1�?2�?3）为数据长度的int值，与版本号组成�?个字�?
        return (r >> 5) & 0xFF;
    }
    ...
    {
        byte v = data[LENGTH_LEN]; //与预留位组成�?个字�?
        return ((v << 3) & 0xFF) >> 3;
    }
    ...
    {
        byte t = data[LENGTH_LEN + VER_LEN];//�?4个字节（0�?1�?2�?3）为数据长度的int值，以及ver占一个字�?
        return t & 0xFF;
    }

    /**
     * 解析接收数据，此处解析了协议版本、协议类型和数据长度，具体内容子类中再解�?
     *
     * @param data
     * @return
